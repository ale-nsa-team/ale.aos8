...
class Radius_serversFacts(object):
    ...
    def parse_radius_servers(self, data):
        ...
        for conf in config:
            match = re.match(r'^aaa\sradius-server\s"(?P<name>(\w+))"\shost\s(?P<host> .*)retransmit\s(?P<retransmit>[\d]+)\stimeout\s(?P<timeout>[\d]+)\sauth-port\s(?P<auth_port>[\d]+)\sacct-port\s(?P<acct_port>[\d]+)\svrf-name\s(?P<vrfname>(\w+))', conf)
            if match:
                members_obj = {
