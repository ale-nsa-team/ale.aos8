...
"""
The aos8_switch_security class
...
class Switch_security(ConfigBase):
    ...
    gather_network_resources = ["switch_security"]

    def __init__(self, module):
        super(Switch_security, self).__init__(module)

    def get_switch_security_facts(self, data=None):
        ...
        switch_security_facts = facts["ansible_network_resources"].get("switch_security")
        if not switch_security_facts:
            return []
        return switch_security_facts
    ...
    def execute_module(self):
        ...
        if self.state in self.ACTION_STATES:
            existing_switch_security_facts = self.get_switch_security_facts()
        else:
            existing_switch_security_facts = []

        # if state in ACTION_STATES, generate commands to device or rendered commands
        if self.state in self.ACTION_STATES or self.state == "rendered":
            commands.extend(self.set_config(existing_switch_security_facts))
        ...
        if self.state in self.ACTION_STATES or self.state == "gathered":
            changed_switch_security_facts = self.get_switch_security_facts()
            ...
            result["parsed"] = self.get_switch_security_facts(data=running_config)
        else:
            changed_switch_security_facts = []

        if self.state in self.ACTION_STATES:
            result["before"] = existing_switch_security_facts
            if result["changed"]:
                result["after"] = changed_switch_security_facts
        elif self.state == "gathered":
            result["gathered"] = changed_switch_security_facts

        result["warnings"] = warnings
        return result

    def set_config(self, existing_switch_security_facts):
        ...
        have = existing_switch_security_facts
    ...
    def _state_replaced(self, want, have):
        ...
        for each in want:
            for every in have:
                if every['access_method'] == each['access_method'] :
                    check = True
                    break
                continue
            if check:
    ...
    def _state_overridden(self, want, have):
        ...
        for each in have:
            ...
            for every in want_local:
                if each['access_method'] == every['access_method']:
                    break
                count += 1
            else:
    ...
    def _set_config(self, want, have):
        ...
        if diff:
            commands.append("aaa authentication " + dict(want).get("access_method") + " " + dict(want).get("aaa_servers_name"))           
    ...
    def _clear_config(self, want, have):
        ...
        commands.append("aaa authentication " + dict(want).get("access_method") + " default")           
