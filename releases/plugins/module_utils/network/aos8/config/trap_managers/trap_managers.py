...
class Trap_managers(ConfigBase):
    """
    The aos8_trap_managers class
    """
    gather_subset = ["!all", "!min"]
    gather_network_resources = ["trap_managers"]

    def __init__(self, module):
        super(Trap_managers, self).__init__(module)

    def get_trap_managers_facts(self, data=None):
        ...
        radius_servers_facts = facts["ansible_network_resources"].get("trap_managers")
    ...
    def execute_module(self):
        ...
        if self.state in self.ACTION_STATES:
            existing_trap_managers_facts = self.get_trap_managers_facts()
        else:
            existing_trap_managers_facts = []

        # if state in ACTION_STATES, generate commands to device or rendered commands
        if self.state in self.ACTION_STATES or self.state == "rendered":
            commands.extend(self.set_config(existing_trap_managers_facts))
        ...
        if self.state in self.ACTION_STATES or self.state == "gathered":
            changed_trap_servers_facts = self.get_trap_managers_facts()
            ...
            result["parsed"] = self.get_trap_managers_facts(data=running_config)
        else:
            changed_trap_servers_facts = []

        if self.state in self.ACTION_STATES:
            result["before"] = existing_trap_managers_facts
            if result["changed"]:
                result["after"] = changed_trap_servers_facts
        elif self.state == "gathered":
            result["gathered"] = changed_trap_servers_facts

        result["warnings"] = warnings
        return result

    def set_config(self, existing_trap_managers_facts):
        ...
        have = existing_trap_managers_facts
    ...
    def _set_config(self, want, have):
        ...
        if diff:
            commands.append("snmp station " + dict(want).get("host") + " " + str(dict(want).get("port")) + " " + dict(want).get("string") + " " + dict(want).get("version") + " " + dict(want).get("state"))           
    ...
    def _clear_config(self, want, have):
        ...
        commands.append("no snmp station " + dict(want).get("host"))           
