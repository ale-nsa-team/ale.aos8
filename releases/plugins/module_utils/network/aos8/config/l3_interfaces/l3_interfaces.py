...
class L3_interfaces(ConfigBase):
    ...
    def _set_config(self, want, have):
        ...
        if diff:
            try:
                isVRF = dict(want).get("vrf")
            except:
                pass
            
            try:
                isLoopback = re.match("Loopback", dict(want).get("name"))
            except:
                pass

            if isVRF:
                commands.append("vrf " + dict(want).get("vrf") + " ip interface " + dict(want).get("name") + " address " + dict(want).get("address") + " mask " + dict(want).get("mask") + " " + dict(want).get("type") + " " + dict(want).get("port_id"))           
                return commands
            
            if isLoopback:
                commands.append("ip interface " + dict(want).get("name") + " address " + dict(want).get("address"))
                return commands

            commands.append("ip interface " + dict(want).get("name") + " address " + dict(want).get("address") + " mask " + dict(want).get("mask") + " " + dict(want).get("type") + " " + dict(want).get("port_id"))           
    ...
    def _clear_config(self, want, have):
        ...
        if isVRF:
            commands.append("vrf " + dict(want).get("vrf") + "no ip interface " + dict(want).get("name"))           
            return commands
        
        if isLoopback:
            commands.append("no ip interface " + dict(want).get("name"))
            return commands
